...
FMP_API_KEY = "5URpNHQqVnWHDFLp5FhKH4LLlC6AFzdj"  # Replace with your actual API key
...
def main():
    ...
    # Add SoFi logo at the top of the sidebar
    sofi_logo_url = "https://i.imgur.com/XUdLcf9.jpeg"  # Replace with your SoFi logo URL
    st.sidebar.image(sofi_logo_url, use_container_width=True)

    # Default tickers in session state
    ...
    **Features:**
    ...
    - Search tickers of stocks by name
    """)

    st.sidebar.header("Tools")

    # Search functionality using FMP API
    st.sidebar.subheader("Search for Stocks by Name")
    search_query = st.sidebar.text_input("Enter company name", placeholder="e.g., Apple, Microsoft")
    if search_query:
        # Fetch matching tickers from FMP API
        url = f"https://financialmodelingprep.com/api/v3/search?query={search_query}&limit=10&apikey={FMP_API_KEY}"
        try:
            ...
            if response.status_code == 200:
                ...
                if matches:
                    ...
                    for match in matches[:5]:  # Show top 5 matches
                        st.sidebar.write(f"{match['name']} ({match['symbol']})")
                else:
                    st.sidebar.write("No matches found.")
            else:
                st.sidebar.write("Error fetching data from API.")
        except Exception as e:
            st.sidebar.write(f"API request failed: {str(e)}")

    # Existing ticker input field
            ...
            with tab1:
                ...
                st.plotly_chart(go.Figure(
                    go.Heatmap(
                        ...
                        x=optimizer.tickers,
                        y=optimizer.tickers,
                        colorscale='RdBu',  # Corrected typo here
                        zmid=0
                    )
            ...
            with tab2:
                ...
                st.plotly_chart(go.Figure(
                    data=[go.Scatter(
                        ...
                        name=col
                    ) for col in optimizer.tickers]
                ), use_container_width=True)
            with tab3:
                st.write("Optimized Portfolio:", {ticker: weight for ticker, weight in zip(optimizer.tickers, optimal_weights)})
                st.write("Expected Return:", round(opt_ret, 4), "Volatility:", round(opt_vol, 4))
